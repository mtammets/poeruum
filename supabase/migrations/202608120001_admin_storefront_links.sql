-- Let platform administrators open each published merchant storefront directly
-- from the user dashboard. Only the primary active custom hostname is exposed;
-- the client falls back to the store's Poeruum subdomain when it is absent.
...
comment on function public.admin_dashboard_users() is
  'Returns merchant setup and activity data, including the active primary storefront hostname, to platform administrators.';
