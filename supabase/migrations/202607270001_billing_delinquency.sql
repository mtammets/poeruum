alter table public.stores
  ...
  add column if not exists billing_last_failed_invoice_id text,
  add column if not exists billing_last_failed_invoice_url text,
  add column if not exists billing_failure_notified_at timestamptz,
  add column if not exists billing_grace_reminder_sent_at timestamptz,
  add column if not exists billing_downgraded_at timestamptz,
  add column if not exists billing_downgrade_notified_at timestamptz,
  add constraint stores_billing_grace_order_check
    check (
      billing_grace_ends_at is null
      or (
        billing_delinquent_at is not null
        and billing_grace_ends_at >= billing_delinquent_at
      )
    );

create index if not exists stores_billing_delinquency_idx
on public.stores(billing_grace_ends_at)
where pricing_plan = 'fixed' and billing_delinquent_at is not null;

comment on column public.stores.billing_delinquent_at is
  'The start of the current failed-payment episode. It is not reset by later retries.';
comment on column public.stores.billing_grace_ends_at is
  'The exclusive end of the fixed-plan entitlement after a recurring payment failure.';
comment on column public.stores.billing_last_failed_invoice_url is
  'Stripe-hosted owner-only recovery URL for the current failed subscription invoice.';

create or replace function public.effective_store_pricing_plan(target_store_id uuid)
returns text
language sql
stable
security definer
...
as $$
  select case
    when store.pricing_plan = 'fixed'
      and (
        store.stripe_subscription_status in ('active', 'trialing')
        or (
          store.stripe_subscription_status in ('past_due', 'unpaid')
          and store.billing_grace_ends_at > now()
        )
      )
    then 'fixed'
    else 'flexible'
...
revoke all on function public.effective_store_pricing_plan(uuid) from public, anon, authenticated;
grant execute on function public.effective_store_pricing_plan(uuid) to service_role;
...
create or replace function public.clear_billing_data_when_owner_removed()
returns trigger
language plpgsql
set search_path = ''
as $$
begin
  if old.owner_id is not null and new.owner_id is null then
    ...
    new.billing_last_failed_invoice_id := null;
    new.billing_last_failed_invoice_url := null;
    new.billing_failure_notified_at := null;
    new.billing_grace_reminder_sent_at := null;
    new.billing_downgraded_at := null;
    new.billing_downgrade_notified_at := null;
  end if;
  return new;
end;
$$;

drop trigger if exists stores_clear_billing_data_on_owner_removal on public.stores;
create trigger stores_clear_billing_data_on_owner_removal
before update of owner_id on public.stores
for each row execute function public.clear_billing_data_when_owner_removed();

revoke all on function public.clear_billing_data_when_owner_removed() from public, anon, authenticated;

do $$
declare existing_job bigint;
begin
  select jobid into existing_job from cron.job where jobname = 'poeruum-billing-delinquency';
  ...
  if not exists (
    ...
    raise notice 'Billing delinquency schedule skipped because Vault secrets are not configured.';
    return;
  end if;
  perform cron.schedule(
    'poeruum-billing-delinquency',
    '*/15 * * * *',
    $schedule$
...
do $$
begin
  if has_column_privilege('authenticated', 'public.stores', 'billing_grace_ends_at', 'UPDATE')
    or has_column_privilege('authenticated', 'public.stores', 'billing_last_failed_invoice_url', 'UPDATE') then
    raise exception 'ACCESS_ASSERTION_FAILED: authenticated can update service-owned billing columns';
  end if;
end;
$$;
