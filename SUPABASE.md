...
Resendi kontot võivad jagada teised rakendused. Poeruumi webhook võtab väljuvate kirjade sündmusi vastu ainult täpse saatja-aadressi järgi: `teavitused@send.poeruum.ee` (ka Supabase Auth SMTP) ning seadistatud `RESEND_FROM_EMAIL`, `OUTREACH_FROM_EMAIL` ja `SUPPORT_AGENT_FROM_EMAIL`. Kuvanimi ja kirja teema päritolu ei tõenda. Kasutajate „Suhtlus” veerg näitab ainult `email_deliveries.source_application = 'poeruum'` kirjeid; `external` ja kontrollimata (`NULL`) kirjed jäävad peidetuks. Kohalikud tellimuste ja kliendiotsingu saatmisfunktsioonid tuvastatakse salvestatud Resendi kirjatunnuse järgi. Sissetulevad klienditoe kirjad kasutavad jätkuvalt saaja-aadressi kontrolli.

Vana kirjajaloo päritolu kontrollimiseks käivita `node scripts/backfill-email-sources.mjs --audit .local-backups/email-sources.json`. Skript loeb Resendist saatja ja kontrollib kirjatunnust ning saajat; see ei saada kirju ega muuda andmebaasi. Algandmed ja tulemused salvestatakse privaatsesse auditifaili. Pärast kontrolli ja migratsiooni `202609230001` rakendamist käivita `node scripts/backfill-email-sources.mjs --apply .local-backups/email-sources.json`. See lisab kinnitatud päritolumärked, säilitab võõrad kirjed ning jätab kontrollimata või vahepeal muudetud kirjed puutumata. Mõlemal käivitamisel peavad saatja keskkonnaseaded vastama serveri seadetele.
