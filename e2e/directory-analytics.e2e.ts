...
test('admin sees overall and store metrics, switches periods, and keeps an unsaved ordering draft', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'Kaubamaja statistika', exact: true })).toHaveCount(0)
  await expect(page.getByText('Kuidas külastajad poode avastavad ja neisse edasi liiguvad.')).toHaveCount(0)
  await expect(page.getByRole('link', { name: /Ava Kaubamaja/ })).toHaveCount(0)
  await expect(page.getByText('Eesti aeg', { exact: false })).toHaveCount(0)
...
test('mobile statistics handles errors and missing history without page overflow', async ({ page }) => {
  ...
  await expect(page.getByText('Varasemate külastuste ajalugu puudub.', { exact: false })).toHaveCount(0)
