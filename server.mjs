...
const directoryNavigationMarkup = (isStory = false) => `<nav class="store-directory__nav" aria-label="Poeruumi Kaubamaja"><a class="store-directory__brand" href="${isStory ? '/' : `https://${platformHost}/`}" aria-label="${isStory ? 'Kaubamaja avaleht' : 'Poeruumi avaleht'}">${brand}<span class="store-directory__brand-rule" aria-hidden="true"></span><span class="store-directory__brand-edition">Kaubamaja</span></a><div class="store-directory__merchant-entry"><a class="store-directory__manage" href="https://${platformHost}/?continue_setup=1">Minu poe haldus</a><a class="store-directory__create" href="https://${platformHost}/#hind" aria-label="Loo oma e-pood"><span class="store-directory__create-full" aria-hidden="true">Loo oma e-pood</span><span class="store-directory__create-short" aria-hidden="true">Loo e-pood</span>${arrowUpRight}</a></div></nav>`
...
function renderStoreDirectory(template, stores) {
  ...
  const products = getStoreDirectoryHighlights(stores).map(({ store, product }) => `<article class="store-directory__product store-directory__product--highlight"><a href="${escapeHtml(getStoreDirectoryVisitUrl(store, product))}" aria-label="${escapeHtml(`${product.name} – ${store.name}`)}"><div class="store-directory__product-media"><img src="${escapeHtml(product.imageUrl)}" alt="" loading="lazy" decoding="async"></div><div class="store-directory__product-copy"><span class="store-directory__product-store">${escapeHtml(store.name)}</span><h3>${escapeHtml(product.name)}</h3><p class="store-directory__product-price"><strong>${escapeHtml(formatStoreDirectoryPrice(product.salePrice ?? product.price))}</strong>${product.salePrice !== null ? `<del>${escapeHtml(formatStoreDirectoryPrice(product.price))}</del>` : ''}</p><span class="store-directory__product-action" aria-hidden="true">Vaata toodet ${arrowUpRight}</span></div></a></article>`).join('')
  const highlights = products ? `<section class="store-directory__highlights" aria-labelledby="directory-products-heading"><div class="store-directory__section-head"><div><h2 id="directory-products-heading">Leide Eesti poodidest</h2><p class="store-directory__highlights-intro">Vali oma lemmik ja vaata lähemalt e-poes.</p></div></div><div class="store-directory__product-grid">${products}</div></section>` : ''
  const content = `<main class="store-directory">${directoryNavigationMarkup()}<header class="store-directory__hero"><div class="store-directory__hero-media" aria-hidden="true"><img src="/images/poeruumi-kaubamaja-hero.webp" alt="" fetchpriority="high" decoding="async"></div><div class="store-directory__intro"><h1>${heading}</h1><p>${description}</p></div></header>${highlights}<section class="store-directory__stores" aria-labelledby="store-directory-heading"><div class="store-directory__section-head"><h2 id="store-directory-heading">Leia oma uus lemmikpood</h2></div><div class="store-directory__grid">${cards}</div>${renderDirectoryStoryTeaser(featuredDirectoryStory)}</section>${directoryFooterMarkup}</main>`
  ...
  try {
    ...
    if (!slug) {
      ...
      if (isPlatformRequest) {
        const isAdminRequest = /^\/admin(?:\/(?:analytics|homepage|seo|leads|users|support|kaubamaja))?\/?$/i.test(url.pathname)
