...
function PlatformFlow() {
  ...
  const trackFaqOpen = (event: React.MouseEvent<HTMLElement>) => {
    ...
    // Summary activation runs before the browser changes the open state and
    // includes keyboard activation, so only a user opening a closed FAQ counts.
    if (!event.nativeEvent.isTrusted || event.defaultPrevented
      || !(details instanceof HTMLDetailsElement) || details.open) return
    const label = details.dataset.analyticsLabel
  ...
  if (screen === 'landing') return <main className="platform-landing">
    ...
    <section className="platform-faq" id="kkk">
      ...
      <div className="platform-faq__list">
        ...
        <details data-analytics-label="google"><summary onClick={trackFaqOpen}>Kas minu pood ja tooted on Google’is leitavad?<span>+</span></summary><p>Jah. Poeruum hoolitseb automaatselt selle eest, et avaldatud pood ja otsingus nähtavad tooted oleksid Google’ile leitavad. Iga poe ja toote jaoks luuakse otsingusõbralik aadress, pealkiri, kirjeldus ning Google’ile vajalik tehniline info. Sina saad nähtavust parandada täpsete tootenimede, sisukate kirjelduste ja kvaliteetsete piltidega. Kõik see toimib ka oma domeeni kasutamisel.</p></details>
        <div className="platform-faq__more" id="faq-more" hidden={!showAllFaq}>
          <details data-analytics-label="mobile_setup"><summary onClick={trackFaqOpen}>Kas saan kogu poe telefonis valmis teha?<span>+</span></summary><p>Jah. Telefonis saad luua konto, pildistada ja lisada tooted, määrata hinnad, seadistada tarne, kujundada poe ning selle avaldada.</p></details>
          <details data-analytics-label="buyer_account"><summary onClick={trackFaqOpen}>Kas ostjal peab olema Poeruumi konto?<span>+</span></summary><p>Ei. Ostja lisab kassas kontakt- ja tarneandmed, tasub Stripe’is ning saab tellimuse kinnituse e-postile.</p></details>
          <details data-analytics-label="order_notice"><summary onClick={trackFaqOpen}>Kuidas saan tellimusest teada ja millal raha laekub?<span>+</span></summary><p>Pärast kinnitatud makset ilmub tellimus poe haldusesse ja soovi korral saadetakse sulle e-kiri. Ostjale saadetakse tellimuse kinnitus. Makset töötleb Stripe ning raha jõuab sinu pangakontole Stripe’i väljamaksegraafiku järgi.</p></details>
          <details data-analytics-label="refunds"><summary onClick={trackFaqOpen}>Kas saan ostjale makse tagastada?<span>+</span></summary><p>Jah. Stripe’iga tasutud tellimuse makse saad tellimuste vaates täielikult tagastada. Kauba tagastamise ja kliendisuhtluse korraldad oma müügitingimuste järgi.</p></details>
          <details data-analytics-label="design"><summary onClick={trackFaqOpen}>Kui palju saan poe kujundust muuta?<span>+</span></summary><p>Poe ilme saad kohandada oma brändile sobivaks. Näiteks saad valida kujunduse ja värvid, lisada logo, muuta nuppude ja soodushindade välimust ning lisada teateriba. Valikuid saad hiljem alati muuta.</p></details>
          <details data-analytics-label="change_plan"><summary onClick={trackFaqOpen}>Kas saan paketti hiljem vahetada?<span>+</span></summary><p>Jah. Paindlikult paketilt saad Kindlale üle minna poe halduses. Kindlalt paketilt Paindlikule minnes jääb senine pakett kehtima Stripe’is näidatud perioodi lõpuni ja seejärel rakendub Paindlik pakett.</p></details>
          <details data-analytics-label="support"><summary onClick={trackFaqOpen}>Kust saan abi?<span>+</span></summary><p>Pärast sisselogimist saad küsimuse saata Abi-nupu kaudu. Vastuseid ja varasemat vestlust näed samas kohas ning vastus saadetakse ka sinu konto e-postile.</p></details>
        </div>
