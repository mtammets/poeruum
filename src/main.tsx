...
const isAdminPath = isPlatformSurface && !isStoreDirectorySurface && /^\/admin(?:\/(?:analytics|seo|leads|users|support|business-card|kaubamaja))?\/?$/i.test(window.location.pathname)
...
function Homepage() {
  ...
  useEffect(() => {
    ...
    client.from('platform_settings')
      .select('seo_title,seo_description,social_title,social_description,search_indexing_enabled,social_image_path')
      .eq('id', 'homepage')
      .maybeSingle()
      .then(({ data }) => {
        ...
        setHomepageSettings({
          ...
          searchIndexingEnabled: data.search_indexing_enabled,
          socialImagePath: data.social_image_path,
        })
      })

    const channel = client.channel('public-homepage-settings')
      .on('postgres_changes', {
        event: 'UPDATE',
        schema: 'public',
